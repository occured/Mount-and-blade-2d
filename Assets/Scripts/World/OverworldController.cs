...
namespace MountAndBlade2D.World
{
    ...
    public class OverworldController : MonoBehaviour
    {
        ...
        [SerializeField] private Character.SprintController sprintController;
        [SerializeField] private OverworldBounds bounds;
        [SerializeField] private Party.PartyTravelSpeed travelSpeed;
        ...
        private void Update()
        {
            if (inputReader != null)
            {
                ...
                {
                    sprintController.SetSprinting(inputReader.SprintHeld);
                }
            }
            else
            {
        ...
        private void FixedUpdate()
        {
            ...
            {
                speed *= sprintController.SprintMultiplier;
            }
            if (travelSpeed != null)
            {
                speed *= travelSpeed.GetSpeedMultiplier();
            }

            var targetPosition = _rb.position + _input.normalized * (speed * Time.fixedDeltaTime);
            if (bounds != null)
            {
                targetPosition = bounds.Clamp(targetPosition);
            }
