...
8. Add `CombatSceneBootstrap` and `CombatResolution` to the combat scene, assign enemy + ally prefabs and spawn roots to instantiate combatants from the last encounter and return to overworld; optionally add `CombatResultUI`/`CombatDefeatUI` + `SceneTransitionFader` for a short victory/defeat pause.
